...
import com.nd.pad.constPackage.Constants;
...
public class ImageDownloader {
     ...
     * 方法描述：下載圖片方法，帶有對應回調
     * @param url 图片下载地址
     * @param imageView 用于显示图片的imageview控件
     * @param loadingDrawable_ID 图片正在下载时控件显示的图片
     * @param failDrawable_ID 图片下载失败后控件显示的图片
     * @param isSaveLocal 图片是否保存在本地
     * @param action 回调接口
     * 项目名称： MyGoPlusV3 
     * <pre>
     * 修改日期      修改人	   修改说明
     ...
     * @param url图片下载地址
     * @param imageView 显示图片用imageview控件
     * @param width 设置图片显示的宽度，压缩后的图片宽度，实际返回宽度大于或等于参数值
     * @param height 设置图片显示的高度，压缩后的图片高度，实际返回高度大于或等于参数值
     * @param loadingDrawable_ID 图片下载中所显示的图片
     * @param failDrawable_ID 图片下载失败后所显示的图片
     * @param isSaveLocal 图片是否保存在本地
     * @param action 回调接口
     * 项目名称： MyGoPlusV3 
     * <pre>
...
//        Bitmap bitmap = getBitmapFromCache(url);
     ...
     */
    private void forceDownload(String url, ImageView imageView, int width, int height, String parentFullFileName, int loadingDrawable_ID,
            int failDrawable_ID, boolean isSaveLocal, ImageDownLoaderAction action) {
	 ...
	 * @param url 图片下载地址
	 * @param width 压缩后的图片宽度，实际返回宽度大于或等于参数值
	 * @param height 压缩后的图片高度，实际返回高度大于或等于参数值
	 * @param parentFullFileName 存储图片的文件夹全路径
	 * @param isSaveLoacl 是否保存本地
	 * @return 图片
	 * 项目名称： MyGoPlusV3 
	 * <pre>
	 * 修改日期      修改人	   修改说明
    ...
    Bitmap downloadBitmap(String url, int width, int height, String parentFullFileName, boolean isSaveLoacl) {
        ...
        final HttpClient client = (mode == Mode.NO_ASYNC_TASK) ? new DefaultHttpClient() : AndroidHttpClient
                .newInstance("Android");
	 ...
	 * 方法描述：从本地那图片
	 * @param url 图片下载地址
	 * @param parentFullFileName 图片所在目录全路径
	 * @param width 需要返回的图片宽度，实际值大于或等于参数值
	 * @param height 需要返回的图片高度，实际值大于或等于参数值
	 * @return 
	 * 项目名称： MyGoPlusV3 
	 * <pre>
	 * 修改日期      修改人	   修改说明
    ...
    * </pre>
    */
   public Bitmap loadResizedBitmap(String filename, double width,
           double height) {
       Bitmap bitmap = null;
	 ...
	 * @param parentFullFileName 保存文件的文件夹全路径
	 * 项目名称： MyGoPlusV3 
	 * <pre>
	 * 修改日期      修改人	   修改说明
	 * 2013-6-14   劉文強		新建
...
//		return str;
	...
	public void removeCache(String parentFullFileName) {
		...
		if (dirSize > Constants.CACHE_SIZE * Constants.MB
				|| Constants.FREE_SD_SPACE_NEEDED_TO_CACHE > freeSpaceOnSd()) {
